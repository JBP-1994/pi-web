...
test("all active-session transitions share one persistence effect", () => {
  assert.match(
    source,
    /useEffect\(\(\) => \{\s+if \(!selectedSession\) return;[\s\S]*?setLastOpenSession\(cwd, selectedSession\.id\);\s+\}, \[selectedSession\]\);/,
  );
});
...
test("workspace restoration remains inside the cross-project branch", () => {
  assert.match(
    ...
    /if \(currentProject !== newProject\) \{[\s\S]*?restoreWorkspaceContext\(cwd\);[\s\S]*?\}/,
  );
});
